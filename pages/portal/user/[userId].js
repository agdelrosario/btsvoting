  ...
  useEffect(() => {
    ...
    const fetchAppAccounts = async () => {
      return Promise.all(apps.map(async (app) => {
        const res = await fetch(`/api/accounts/${app.slug}?userId=${router.query.userId}`)
      ...
      }
      {
        ...
        >
          {/* <div> */}
            ...
            <VotingGrid userId={session.id} validation={validationVoting} setValidation={setValidationVoting} apps={apps} appAccounts={appAccounts} disableEdit={true} />
...
export async function getServerSideProps(ctx) {
  ...
  const profileRes = await fetch(`${process.env.HOST}/api/profiles/single?userId=${ctx.query.userId}`);
